...
function findSeniors(persons) {
	const webDev = persons.filter(
		(persons) => persons.experience >= 5 && persons.job === "web dev",
	);
	const dataAnalyst = persons.filter(
		(persons) => persons.experience >= 5 && persons.job === "data analyst",
	);
